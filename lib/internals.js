		...
		switch (typeof body) {
			...
			case 'object':
				...
				if ( ! haveBodyHandler) {
					...
					if (body.options) {
						if ( ! isNaN(Number(body.options.start))) {
							if (body.options.start < 0) {
								body.options.start = 0;
							}
							if (body.options.start >= options.headers['content-length']) {
								var err = new Error('The start value of a byte range can\'t be more that the file length.');
								callback(err);
								return;
							}
						}
						if ( ! isNaN(Number(body.options.end))) {
							if (body.options.end > (options.headers['content-length'] - 1)) {
								body.options.end = options.headers['content-length'] - 1;
							}
							if (body.options.end < 0) {
								var err = new Error('The end value of a byte range can\'t be less than 0.');
								callback(err);
								return;
							}
						}
						if (Number(body.options.end) > 0 && isNaN(Number(body.options.start))) {
							body.options.start = 0; // the node.js upstream is dumb about this ...
						}
						if (Number(body.options.start) >= 0 && isNaN(Number(body.options.end))) {
							body.options.end = options.headers['content-length'] - 1;
						}
						if (body.options.start > body.options.end) {
							var err = new Error('The start value of a byte range must be lower than the end value.');
							callback(err);
							return;
						}
						if ( ! isNaN(body.options.start) && ! isNaN(body.options.end)) {
							options.headers['content-length'] = body.options.end - body.options.start + 1;
						} else {
							delete(body.options.start);
							delete(body.options.end);
						}
					}
				}
			break;
