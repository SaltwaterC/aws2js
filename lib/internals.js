 ...
 * TODO: break it in smaller pieces - it's over 9000 LOC
 *
 * @param {Object} config
...
var makeRequest = function(config, options, body, handler, callback, requestId) {
	...
	tools.debug(options.method + ' ' + options.path);
	tools.debug('host: ' + options.host);

	var header;
	for (header in options.headers) {
		if (options.headers.hasOwnProperty(header)) {
			tools.debug(header + ': ' + options.headers[header]);
		}
	}
	...
	// TODO: refactor the whole S3 stuff to drop the host based addressing
	// in favor of path based addressing in order to have proper HTTPS support
	if (config.prefix === 's3' && String(config.useBucket).indexOf('.') !== -1) {
		tools.debug('disabled the SSL validation for bucket: ' + config.useBucket);
	...
	var request = https.request(options, function(response) {
		...
		if (response.statusCode === 307) { // oh great ... S3 crappy redirect
			...
			var checkBoundary = function(size) {
	...
	if (body) { // aka we have a request body to handle
		if (config.noContinue) {
			// code for handling retarded services like SNS, STS, DynamoDB
			request.write(body);
			request.end();
		} else {
			// code for the rest of the services that handle properly Expect: 100-continue
			// aka don't hand you back a 500 error
...
var putLifeCycleConfig = function(config, lifecycle, callback) {
	...
	for (i = 0; i < lifecycle.Rule.length; i++) {
		body += '<Rule><ID>' + lifecycle.Rule[i].ID + '</ID><Prefix>' + lifecycle.Rule[i].Prefix + '</Prefix><Status>' + lifecycle.Rule[i].Status + '</Status><Expiration><Days>' + lifecycle.Rule[i].Expiration.Days + '</Days></Expiration></Rule>';
	}
