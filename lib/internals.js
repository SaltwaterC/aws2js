...
var makeRequest = function (config, options, body, handler, callback, requestId) {
	...
	if (body) {
		switch (typeof body) {
			...
			break;
			default:
				callback(new Error('Invalid request body handler. Expecting a String or Object.'));
				return;
			break;
		}
	}
	...
	if (handler != 'xml' && handler != 'buffer' && handler != 'stream' && handler != null) {
		try {
			if ( ! handler.file) {
				callback(new Error('Invalid response body handler. Expecting an object containing a file path.'));
				return;
			}
