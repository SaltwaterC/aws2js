...
// TODO: refactor all this crap in v0.7. It's time.
...
var client = function (config, httpOptions) {
	 ...
	 * TODO: migrate to lodash.clone() in 0.7
	 *
	 * @returns {Object}
	 */
	config.cloneHttpOptions = function () {
		var key, result = {};
		for (key in httpOptions) {
	...
	if (config.prefix !== 's3') {
		...
		config.request = function (action, query, callback) {
			...
			options = config.cloneHttpOptions();
			options.method = 'POST';
	...
	if (config.prefix === 's3') {
		 ...
		 * May be the same bucket, but it's easier with s3.renameObject()
		 * 
		 * @param {String} source
		 * @param {String} destination
		 * @param {String} acl
...
exports.load = function (service, accessKeyId, secretAccessKey, sessionToken, httpOptions) {
	...
	var clientTemplate = cfg.clients[service], clientConfig = {}, key;
	if ( ! clientTemplate) {
	...
	// use lodash.clone() in v0.7
	for (key in clientTemplate) {
