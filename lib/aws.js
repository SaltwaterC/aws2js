	...
	var makeRequest = function (options, body, handler, callback, requestId) {
		...
		if (body) {
			if (typeof body == 'string') {
				...
				var bfile = fs.createReadStream(bf);
				...
				bfile.on('error', function (error) {
					...
					process.on('uncaughtException', function (error) {
						/**
						 * This crap is here due to a bug into the node.js
						 * HTTPS client.
						 * See: https://github.com/joyent/node/issues/1304
						 */
					});
				});
			}
	...
	if (config.prefix == 's3') {
		...
		var standardHeaders = function (method, headers, path) {
			...
			/*
				...
				if ( ! haveType) {
					options.headers['Content-Type'] = mime.lookup(bf);
				}
				
				* implement the above code if the request body handler is file
				* make all headers to be lower case
				* implement content-md5 if the request body handler is file
				* simplify the authorize() function
			*/
