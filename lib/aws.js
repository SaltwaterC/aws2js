...
var client = function (config) {
						...
						default:
							if (response.statusCode != 200) {
								...
								try {
									...
									var expectAssertError = false;
									if (request.socket.pair.ssl.receivedShutdown == true) {
										expectAssertError = true;
									}
									request.abort(); // this should be enough, but read below
									if (expectAssertError) {
										process.once('uncaughtException', function (error) {
											/**
											 * This crap is here due to a bug into the node.js HTTP client.
											 * @link https://github.com/joyent/node/issues/1085
											 */
										});
									}
		...
		if (body) {
			if (typeof body == 'string') {
				...
				bfile.on('error', function (error) {
					...
					process.once('uncaughtException', function (error) {
						/**
						 * This crap is here due to a bug into the node.js HTTPS client.
						 * @link https://github.com/joyent/node/issues/1304
						 */
					});
				});
			}
	...
	/**
	 * Accessible by query APIs except SES
	 */
