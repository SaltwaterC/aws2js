...
var client = function (config) {
	...
	if (config.prefix == 's3') {
		...
		config.del = function (path, callback) {
			...
			if ( ! config.useBucket) {
				var error = new Error('The DELETE method requires the usage of s3.setBucket().');
				callback(error);
				return;
			}
		...
		config.put = function (path, headers, body, callback) {
			...
			internals.standardHeaders(config, 'PUT', tools.merge(headers, {
					'content-length': 0
				}), path, body, function (err, hdrs) {
					if (err) {
						callback(err);
					} else {
						internals.makeRequest(config, {
		...
		config.putObject = function () {
			// This is deprecated due to ambiguity when using a file path vs. a buffer which represents a file path
