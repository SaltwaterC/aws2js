...
var client = function (config, httpOptions) {
  ...
  if (config.prefix === 'cloudfront') {
    ...
    config.getDistributions = function (callback) {
      config.get(config.distributionPath(), 'xml', callback);
    };
    ...
    config.invalidate = function (distributionId, docPathOrPaths, reference, callback) {
      ...
      document.node('CallerReference', reference);
    ...
    config.getInvalidations = function (distributionId, callback) {
      config.get(config.invalidationPath(distributionId), 'xml', callback);
    };
