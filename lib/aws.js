...
var client = function (config, httpOptions) {
  ...
  if (config.prefix === 'cloudfront') {

    config.getDistributions = function (query, callback) {
      if (!callback) {
        callback = query;
        query = {};
      }
      config.get(config.distributionPath(), query, 'xml', callback);
    };

    config.getDistribution = function (distributionId, callback) {
      config.get(config.distributionPath(distributionId), 'xml', callback);
    };

    config.invalidate = function (distributionId, docPathOrPaths, reference, callback) {
      var document = new libxmljs.Document().node('InvalidationBatch');
      if (typeof docPathOrPaths === 'string') {
        docPathOrPaths = [docPathOrPaths];
      }
      var items = document.node('Paths').
        node('Quantity', docPathOrPaths.length.toString()).parent().
        node('Items');
      docPathOrPaths.forEach(function (path) {
        items.node('Path', path);
      });
      document.node('CallerReference', reference.toString());
      ...
      config.post(config.invalidationPath(distributionId), headers, body, callback);
    };

    config.getInvalidations = function (distributionId, query, callback) {
      if (!callback) {
        callback = query;
        query = {};
      }
      config.get(config.invalidationPath(distributionId), query, 'xml', callback);
    };

    config.getInvalidation = function (distributionId, invalidationId,  callback) {
      config.get(config.invalidationPath(distributionId, invalidationId), 'xml', callback);
    };

    config.distributionPath = function (distributionId) {
      return '/' + config.query.Version + '/distribution' + (distributionId ? '/' + distributionId : '');
    };

    config.invalidationPath = function (distributionId, invalidationId) {
      return '/' + config.query.Version + '/distribution/' + distributionId + '/invalidation' + (invalidationId ? '/' + invalidationId : '');
    };
  }

	/* in use by the S3 REST API */
	
	if (config.prefix === 's3' || config.prefix === 'cloudfront') {
