...
var client = function(config) {
	...
	if (config.prefix != 's3') {
		/**
		 * Gets the defined end point for query API requests
		 */
		...
		config.call = function(action, query, callback) {
			if ( ! internals.checkConfig(config, callback)) {
				return;
			}
			query.Action = action;
	...
	if (config.prefix == 's3') {
		...
		config.get = function(path, handler, callback) {
			...
			internals.makeRequest(config, {
				method: 'GET',
				path: path,
				headers: internals.standardHeaders(config, 'GET', {}, path)
		...
		config.head = function(path, callback) {
			...
			internals.makeRequest(config, {
				method: 'HEAD',
				path: path,
				headers: internals.standardHeaders(config, 'HEAD', {}, path)
			}, false, null, callback);
		};
		/**
		...
		config.del = function(path, callback) {
			...
			internals.makeRequest(config, {
				method: 'DELETE',
				path: path,
				headers: internals.standardHeaders(config, 'DELETE', {
					'content-length': 0
				}, path)
			}, false, null, callback);
		};
		...
		config.put = function(path, headers, body, callback) {
			...
			internals.makeRequest(config, {
				method: 'PUT',
				path: path,
				headers: internals.standardHeaders(config, 'PUT', tools.merge(headers, {
					'content-length': 0
				}), path, body)
			}, body, null, callback);
		};
